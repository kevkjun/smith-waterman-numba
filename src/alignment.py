...
if __name__ == "__main__":
    ...
    parser.add_argument('--outfile', type=str, default='./alignment.txt')
    
    args = parser.parse_args()

    header, seq = next(fp.parse_protein(args.infile))
    gen = fp.parse_protein(args.db)
    db = args.db.split('/')[-1]
    newline = '\n'
    ...
    with open(args.outfile, "w") as f:
        ...
        scores = [f'> {GlobalAligner(seq, j_seq, args.matrix, args.gap).alignment_score} | {desc}{newline}' for desc, j_seq in gen] if args.scope == 'global' \
            else [f'> {LocalAligner(seq, j_seq, args.matrix, args.gap).alignment_score} | {desc}{newline}' for desc, j_seq in gen]
