...
class matrices():
    ...
    b62 = np.zeros((26,26), dtype=np.int64)
    A_ord = int(ord('A'))
    for key in list(blosum62.keys()):
        b62[int(ord(key[0])) - A_ord][int(ord(key[1])) - A_ord] = blosum62.get(key)
        b62[int(ord(key[1])) - A_ord][int(ord(key[0])) - A_ord] = blosum62.get(key)

    # get the BLOSUM62 matrix into a numpy array
    p250 = np.zeros((26,26), dtype=np.int64)
    for key in list(pam250.keys()):
        p250[int(ord(key[0])) - A_ord][int(ord(key[1])) - A_ord] = pam250.get(key)
        p250[int(ord(key[1])) - A_ord][int(ord(key[0])) - A_ord] = pam250.get(key)