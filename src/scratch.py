...
# print(list(blosum62.keys()))
# for key in sorted(set(list(blosum62.keys()))):
#     print(key[0])

# print(len(blosum62))
# print(26*26)

# # get all the keys and add them to list to get just the unique ones
# k = set()
# for key in list(blosum62.keys()):
#     k.add(key[0])
# k_list = list(k)
# k_list = sorted(k_list)

# # after alphabetizing, insert * for J, O, U
# k_list.insert(9, '*')
# k_list.insert(14, '*')
# k_list.insert(20, '*')

# char = np.array(k_list)
# print(char)
# print(char.shape)
# print(char.reshape(-1, 1))

# # get the BLOSUM62 matrix into a numpy array
# b62 = np.zeros((26,26))
# A_ord = ord('A')
# for key in list(blosum62.keys()):
#     b62[ord(key[0]) - A_ord][ord(key[1]) - A_ord] = blosum62.get(key)
#     b62[ord(key[1]) - A_ord][ord(key[0]) - A_ord] = blosum62.get(key)
# print(b62)

# # get the BLOSUM62 matrix into a numpy array
# p250 = np.zeros((26,26))
# A_ord = ord('A')
# for key in list(pam250.keys()):
#     p250[ord(key[0]) - A_ord][ord(key[1]) - A_ord] = pam250.get(key)
#     p250[ord(key[1]) - A_ord][ord(key[0]) - A_ord] = pam250.get(key)
# print(p250)

# l = ['serena', 'kevin','leah']
# k = [np.array(list(name)) for name in l]
# print(k)
# h = np.array(k)
# print(h)
# print(h[0][4])
...
# print(numba_str(np.array([116, 101, 120, 116])))
# print(np.array([116, 101, 120, 116]).shape)
# print(ord('A'))
# db = ['text', 'tfxt']
# db = np.array([np.array([ord(char) for char in db_seq]) for db_seq in db])
# print(db)

# alignment_seq = 'text'
# alignment_seq = np.array([ord(char) for char in alignment_seq])
# print(alignment_seq)
# db = 129
# num_threads = 64
# num_blocks = db//64 if db % 64 == 0 else db//64 + 1
# print(num_blocks)

db_seqs = []
for record in SeqIO.parse('../data/excerpt_pdbaa.fasta', "fasta"):
    db_seqs.append(record.seq)

np_db_seqs = np.array([np.array([ord(char) for char in db_seq]) for db_seq in db_seqs])
print(np_db_seqs)

np_db_seqs = [[ord(char) for char in db_seq] for db_seq in db_seqs]
d = np.array(np_db_seqs)
print(d)